...
import {
  ...
  CardTitle,
  CardContent,
} from "@/components/ui/card"; // Ajout CardContent
import {
  Accordion,
  AccordionContent,
  AccordionItem,
...
import {
  ...
  ModuleMetadata,
  LessonMetadata,
} from "@/lib/data/modules"; // Supposons que LessonMetadata existe
...
const findNextLesson = (modules: ModuleMetadata[]) => {
  const currentModule = modules.find(
    (module) => module.status !== "locked" && !module.completed
  );
...
function RoadmapLessonItem({
  lesson,
  moduleId, // Renommé de moduleSlug à moduleId pour la cohérence
  ...
  const getIcon = () => {
    if (isLocked)
      return <Lock className="h-4 w-4 text-muted-foreground/50 shrink-0" />;
    if (lesson.completed)
      return <CheckCircle className="h-4 w-4 text-green-500 shrink-0" />;
    if (isNextUp)
      return (
        <PlayCircle className="h-4 w-4 text-primary shrink-0 animate-pulse" />
      ); // Utiliser couleur primary
    return <Circle className="h-4 w-4 text-muted-foreground/30 shrink-0" />;
  };

  // Url correcte utilisant moduleId et lesson.slug
  const lessonUrl = `/student/learn/modules/${moduleId}/${lesson.slug}`;
  ...
  return (
    ...
    >
      <Link
        ...
        className={cn(
          "flex items-center gap-3 group flex-grow",
...
export default async function RoadmapPage() {
  // Rendre la fonction async si besoin de fetch user data
  ...
  const currentModule = userModulesData.find(
    (m) =>
      m.status === "in_progress" || (!m.completed && m.status === "available")
  );
          ...
          <CardContent className="p-5 sm:p-6">
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex-grow">
                ...
                <Progress
                  value={globalCompletionPercent}
                  className="w-full h-2.5"
                  indicatorClassName="bg-gradient-to-r from-blue-400 to-primary"
                />
              </div>
            ...
            <CardHeader>
              <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                ...
                >
                  ...
                  <Link
                    href={`/student/learn/modules/${
                      nextLessonInfo.module.moduleId
                    }/${
                      nextLessonInfo.lesson.slug ?? nextLessonInfo.lesson.id
                    }`}
                  >
                  ...
                  <div className="grid gap-2">
                    {module.lessons.map((lesson) => (
                      <RoadmapLessonItem
                        ...
                        lesson={lesson}
                        moduleId={module.moduleId} // Utiliser moduleId au lieu de moduleSlug
                        ...
                        isNextUp={
                          ...
                          (nextLessonInfo.lesson.slug ??
                            String(nextLessonInfo.lesson.id)) ===
                            (lesson.slug ?? String(lesson.id))
                        }
                      />
                    ))}
                  </div>
